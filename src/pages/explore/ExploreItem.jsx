        ...
        <div className='content-explore'>
          <div className='top-content-explore'>
            <div className='info-user'>
              <img style={{ cursor: 'pointer' }} src={user?.profile?.avatarUrl !== ('string' && '') ? user?.profile?.avatarUrl : account}
              ...
              <p style={{ cursor: 'pointer' }} className={`fl ${isFollow ? 'isFollow' : ''}`} onClick={handleClickFollow}>
            ...
            <div className='icon-loadMore'>
              <img style={{ cursor: 'pointer' }} src={more} alt='more' onClick={() => setShowMore(true)} />
            </div>
          </div>
            ...
            {postComments.map((comment, index) => (
              <div className='comment-item' key={index}>
                <div className='info-user-comment'>
                  <img src={comment.user.avatarUrl || account} alt='avatar-info-user-comment' />
                </div>
          ...
          <div className='bottom-explore'>
            <div className='bottom1-explore'>
                  ...
                  <span onClick={() => setLoved(!loved)}>
                    {isLiked ? (
                      <i
                        ...
                        style={{ color: '#f20d0d', scale: '1.1', cursor: 'pointer' }}
              ...
              <div className='count-liked'>
                <p>{Object.keys(likes).filter(id => id === explore.id).length} {t('liked')}</p>
              </div>
            </div>

            <div className='bottom2-explore'>
              <img style={{ cursor: 'pointer' }} src={icon} alt='icon' onClick={() => setShowEmoji(!showEmoji)} />
              <textarea
                ...
                ref={inputRef} />
              <p style={{ cursor: 'pointer' }} className={input !== '' ? 'send' : "notSend"} onClick={handleSend}>{t('send')}</p>
            </div>
