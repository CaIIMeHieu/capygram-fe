          ...
          {input.trim() === '' && (
            <>
              <div className='gr-title'>
                <p><b>{t('recent')}</b></p>
                <p style={{ cursor: "pointer" }} className={`p2 ${recentSearches.length > 0 ? 'clear' : ''}`} onClick={clearAllRecentSearches}>{t('clear')}</p>
              </div>
                ...
                {recentSearches.map(search => (
                  ...
                  <li key={search.index}>
                    <div className='recent'>
                      <div className='gr1'>
                        <img src={clock} />
                        <p style={{ cursor: 'pointer' }}>{search.name}</p>
                      </div>
            ...
            input.trim() !== '' && searchResults !== null && (
              ...
              >
                <div className='result'>
                  <div className='gr1'>
                    <img style={{ cursor: 'pointer' }} src={searchResults?.profile?.avatarUrl !== ('string' && '') ? searchResults?.profile?.avatarUrl : account} alt='avatar-user' />
                  </div>
                  <div className='gr2'>
                    <p style={{ cursor: 'pointer' }}><b>{searchResults?.userName}</b></p>
                    <p className='fullname' style={{ cursor: 'pointer' }}>{searchResults?.profile?.fullName}</p>
                  </div>
                </div>
              </div>
            )
