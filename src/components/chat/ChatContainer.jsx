      ...
      <div className='chat-header'>
        <div className='user-details'>
          <div className='avatar'>
            <img style={{ cursor: 'pointer' }} src={currentChat.avatarUrl !== ('string' && '') ? currentChat.avatarUrl : account} alt='avatar' />
          </div>
          <div className='username'>
            <p style={{ cursor: 'pointer' }}><b>{currentChat.fullname}</b></p>
          </div>
        </div>
      ...
      <div className='chat-message'>
        <div className='top-chat-message'>
          <div className='infor'>
            ...
            <button style={{ cursor: 'pointer' }} className='btn-viewProfile' onClick={handleClickViewProfile}><b>{t('viewProfile')}</b></button>
          </div>
        </div>
            ...
            displayMessages.length > 0 && displayMessages.map((msg, index) => (
              <div className='chat' key={index} ref={scrollRef}>
                <div className={`message-item ${msg.type}`}>
                  <div className='avatar'>
                    <img style={{ cursor: 'pointer' }} src={msg.type === 'sent' ?
                  ...
                  <div className={`message-content ${(msg.isReceiverDeleted === true || msg.isSenderDeleted === true) ? 'recalled' : ''}`}>
                    <p>{(msg.isReceiverDeleted === false && msg.isSenderDeleted === false) ? msg.content : t('recalled')}</p>
                    <p className="hours">
                ...
                {showConfirmDelete && (
                  <div className='overlay' onClick={handleCancelConfirmDelete}>
                    ...
                    >
                      <ConfirmDeleteMessage onCancel={handleCancelConfirmDelete} message={msg} deleteMessage={deleteMessage} />
                    </motion.div>
                  </div>
                )}
              </div>
